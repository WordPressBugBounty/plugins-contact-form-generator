...
if($id != 0) {
    //get the rows
    $sql = $wpdb->prepare(
        "SELECT * FROM {$wpdb->prefix}cfg_fields WHERE id = %d",
        $id
    );
...
//set variables
$wpcfg_name = $id == 0 || $row->name == '' ? '' : esc_attr($row->name);
...
$wpcfg_width = $id == 0 || $row->width == '' ? '' : esc_attr($row->width);
...
$wpcfg_select_default_text = $id == 0 || $row->select_default_text == '' ? 'Select' : esc_attr($row->select_default_text);
$wpcfg_select_no_match_text = $id == 0 || $row->select_no_match_text == '' ? 'No results match' : esc_attr($row->select_no_match_text);
$wpcfg_select_show_scroll_after = $id == 0 || $row->select_show_scroll_after == 0 ? 10 : esc_attr(intval($row->select_show_scroll_after));
$wpcfg_select_show_search_after = $id == 0 || $row->select_show_search_after == 0 ? 10 : esc_attr(intval($row->select_show_search_after));
$wpcfg_upload_button_text = $id == 0 || $row->upload_button_text == '' ? 'Select files...' : esc_html($row->upload_button_text);
$wpcfg_upload_minfilesize = $id == 0 || $row->upload_minfilesize == 0 ? 1 : esc_attr(intval($row->upload_minfilesize));
$wpcfg_upload_maxfilesize = $id == 0 || $row->upload_maxfilesize == 0 ? 5 : esc_attr(intval($row->upload_maxfilesize));
...
$wpcfg_tooltip_text = $id == 0 ? '' : esc_attr($row->tooltip_text);
$wpcfg_field_margin_top = $id == 0 ? '' : esc_attr($row->field_margin_top);
...
$wpcfg_datepicker_number_months = $id == 0 ? 1 : esc_attr($row->datepicker_number_months);
$wpcfg_datepicker_mindate = $id == 0 ? '' : esc_attr($row->datepicker_mindate);
$wpcfg_datepicker_maxdate = $id == 0 ? '' : esc_attr($row->datepicker_maxdate);
...
$wpcfg_heading = $id == 0 ? '' : esc_attr($row->heading);
$wpcfg_google_maps = $id == 0 ? '' : esc_html($row->google_maps);
$wpcfg_recaptcha_site_key = $id == 0 ? '' : esc_attr($row->recaptcha_site_key);
$wpcfg_recaptcha_security_key = $id == 0 ? '' : esc_attr($row->recaptcha_security_key);
...
$wpcfg_contact_data_width = $id == 0 ? 120 : esc_attr($row->contact_data_width);
...
<div style="overflow: hidden;margin: 0 0 10px 0;float: right;">
	<div style="float:right;">
		...
		<?php }?>
		<a href="admin.php?page=cfg_fields&filter_form=<?php echo esc_attr($filter_form); ?>"  class="button"><?php echo $t = $id == 0 ? 'Cancel' : 'Close';?></a>
	</div>
</div>
...
<table class="wpcfg_table">
		...
		<td>
			<select id="wpcfg_column_type" name="column_type">
				...
				$col_types = array("0"=>"Both Columns", "1"=>"Left Column : PRO feature","2"=>"Right Column : PRO feature");
					foreach($col_types as $key => $col_type) {
						$selected = $key == $wpcfg_column_type ? 'selected="selected"' : '';
						echo '<option value="' . esc_attr($key) . '" '.$selected.'>' . esc_html($col_type) . '</option>';
					}
				?>
			</select>
		</td>	
	...
	<tr>
		...
		<td><input name="select_show_scroll_after" id="wpcfg_select_show_scroll_after" type="text" value="<?php echo esc_attr($wpcfg_select_show_scroll_after);?>" /></td>
	</tr>
		...
		<td>
			<select id="wpcfg_datepicker_date_format" name="datepicker_date_format">
				...
				$cfg_opts = array("mm/dd/yy"=>"Default - mm/dd/yy", "yy-mm-dd"=>"ISO 8601 - yy-mm-dd","dd/mm/yy"=>"Custom - dd/mm/yy","d M, yy"=>"Short - d M, yy","d MM, yy"=>"Medium - d MM, yyn","DD, d MM, yy"=>"Full - DD, d MM, yy");
					foreach($cfg_opts as $key => $cfg_opt) {
						$selected = $key == $wpcfg_datepicker_date_format ? 'selected="selected"' : '';
						echo '<option value="' . esc_attr($key).'" '.$selected.'>' . esc_html($cfg_opt) . '</option>';
					}
				?>
			</select>
		</td>	
		...
		<td>
			<select id="wpcfg_datepicker_animation" name="datepicker_animation">
				...
				$cfg_opts = array("show"=>"Show (Default)","slideDown"=>"Slide Down","fadeIn"=>"FadeIn","blind"=>"Blind (UI Effect)","bounce"=>"Bounce (UI Effect)","clip"=>"Clip (UI Effect)","drop"=>"Drop (UI Effect)","fold"=>"Fold (UI Effect)","slide"=>"Slide (UI Effect)");
					foreach($cfg_opts as $key => $cfg_opt) {
						$selected = $key == $wpcfg_datepicker_animation ? 'selected="selected"' : '';
						echo '<option value="' . esc_attr($key).'" '.$selected.'>' . esc_html($cfg_opt) . '</option>';
					}
				?>
			</select>
		...
		<td>
			<select id="wpcfg_datepicker_show_icon" name="datepicker_show_icon">
				...
				$cfg_opts = array("0"=>"No","1"=>"Yes");
					foreach($cfg_opts as $key => $cfg_opt) {
						$selected = $key == $wpcfg_datepicker_show_icon ? 'selected="selected"' : '';
						echo '<option value="' . esc_attr($key).'" '.$selected.'>' . esc_html($cfg_opt).'</option>';
					}
				?>
			</select>
	...
	<tr>
		...
		<td>
			<select id="wpcfg_datepicker_input_readonly" name="datepicker_input_readonly">
				...
				$cfg_opts = array("0"=>"No","1"=>"Yes");
					foreach($cfg_opts as $key => $cfg_opt) {
						$selected = $key == $wpcfg_datepicker_input_readonly ? 'selected="selected"' : '';
						echo '<option value="' . esc_attr($key) . '" '.$selected.'>' . esc_html($cfg_opt) . '</option>';
					}
				?>
			</select>
	...
	<tr>
		...
		<td>
			<select id="wpcfg_datepicker_changemonths" name="datepicker_changemonths">
				...
				$cfg_opts = array("0"=>"No","1"=>"Yes");
					foreach($cfg_opts as $key => $cfg_opt) {
						$selected = $key == $wpcfg_datepicker_changemonths ? 'selected="selected"' : '';
						echo '<option value="' . esc_attr($key) . '" '.$selected.'>' . esc_html($cfg_opt) . '</option>';
					}
				?>
			</select>
	...
	<tr>
		...
		<td>
			<select id="wpcfg_datepicker_changeyears" name="datepicker_changeyears">
				...
				$cfg_opts = array("0"=>"No","1"=>"Yes");
					foreach($cfg_opts as $key => $cfg_opt) {
						$selected = $key == $wpcfg_datepicker_changeyears ? 'selected="selected"' : '';
						echo '<option value="' . esc_attr($key) . '" '.$selected.'>' . esc_html($cfg_opt) . '</option>';
					}
				?>
			</select>
		...
		<td>
			<select id="wpcfg_recaptcha_theme" name="recaptcha_theme">
				...
				$cfg_opts = array("light"=>"Light","dark"=>"Dark");
					foreach($cfg_opts as $key => $cfg_opt) {
						$selected = $key == $wpcfg_recaptcha_theme ? 'selected="selected"' : '';
						echo '<option value="' . esc_attr($key).'" '.$selected.'>'.esc_html($cfg_opt).'</option>';
					}
				?>
			</select>
		...
		<td>
			<select id="wpcfg_recaptcha_type" name="recaptcha_type">
				...
				$cfg_opts = array("image"=>"Image","audio"=>"Audio");
					foreach($cfg_opts as $key => $cfg_opt) {
						$selected = $key == $wpcfg_recaptcha_type ? 'selected="selected"' : '';
						echo '<option value="' . esc_attr($key).'" '.$selected.'>'.esc_html($cfg_opt).'</option>';
					}
				?>
			</select>
...
<input type="hidden" name="cfg_token" value="<?php echo $session_cfg_token;?>" id="cfg_token">
</form>
<?php
//options
...
echo '<div style="margin: 2px;">';
    ...
    $order_by = ($wpcfg_ordering_field == 0) ? 'spo.ordering' : 'spo.name';
	...
	echo '<div class="options_wrapper">';
		...
		echo '<div class="menus_header">';
			echo  '<img src="' . esc_url(plugins_url( "../images/new_page.gif" , __FILE__ )) . '" class="new_submenu_img" title="New Option" menu_id="' . esc_attr($id) . '" />';
			if (sizeof($childs_array) == 0) {
				echo  '<img src="' . esc_url(plugins_url( "../images/country.png" , __FILE__ )) . '" class="load_countries_data" title="Load countries data? (239 countries)" />';
			}
		echo '</div>';
		...
		if (sizeof($childs_array) > 0)
		{
			foreach ($childs_array as $key => $value)
			{
                ...
                $title = esc_attr($show_title);

                echo '<li class="' . esc_attr($class) . '" id="option_li_' . $id . '">';
                    echo '<div class="option_item" id="option_' . $id . '">' . $name . '</div>';
                    echo '<div class="menu_moove ' . esc_attr($disabled_ordering) . '" title="Move option">&nbsp;</div>';
                    echo '<div id="edit_' . $id . '" menu_id="' . $id . '" class="edit" title="Edit option">&nbsp;</div>';
                    echo '<div id="showrow_' . $id . '" menu_id="' . $id . '" class="' . esc_attr($show_class) . '" title="' . $title . '">&nbsp;</div>';
                    echo '<div id="remove_' . $id . '" menu_id="' . $id . '" class="delete" title="Delete option">&nbsp;</div>';
                echo '</li>';
			}
		}
		echo '</ul>';

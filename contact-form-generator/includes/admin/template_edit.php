...
$id = intval($id);
$plugin_path = esc_url(WPCFG_PLUGIN_PATH);

if($id != 0) {
//get the rows
    $sql = $wpdb->prepare(
        "SELECT * FROM {$wpdb->prefix}cfg_templates WHERE id = %d",
        $id
    );
...
function create_accordion($txt,$state,$title='',$roll='') {
    ...
    echo '<tr>
			<td colspan="2">
				...
				<div id="scroll_to_'.esc_attr($roll).'" class="temp_block '.esc_attr($state).'" title="'.esc_attr($title).'">'.esc_html($txt).'</div><div style="'.esc_attr($dis).'margin-bottom:6px;">
					<table>';
}
...
function echo_font_tr($txt,$i,$value) {
    ...
    echo '</label>
            ...
            <td class="st_td">
	               <input class="temp_family" value="' . esc_attr($value) . '" name="styles[' . intval($i) . ']" roll="' . intval($i) . '"  id="elem-' . intval($i) . '"/>	               
            </td>
        </tr>
	';
}
function echo_select_tr($txt,$i,$values,$value) {
    ...
    echo '</label>
            ...
            <td class="st_td">
	               <select name="styles[' . intval($i) . ']"  id="elem-' . intval($i) . '" class="temp_select">';
    foreach ($values as $k => $val) {
        $selected = $value == $k ? 'selected="selected"' : '';
        echo '<option value="' . intval($k) . '" ' . $selected . '>' . esc_html($val) . '</option>';
    }
    echo '</select>	               
            </td>
        </tr>
...
function echo_select_tr_with_optgroups($txt,$i,$values,$value) {
    ...
    foreach ($values as $label => $val_array) {
        ...
        foreach ($val_array as $k => $val) {
            ...
            $selected = $value == $k ? 'selected="selected"' : '';
            echo '<option class="' . esc_attr($def_class) . '" value="' . intval($k) . '" ' . $selected . '>' . esc_html($val) . '</option>';
        }
        echo '</optgroup>';
        $q++;
    }
...
function echo_color_tr($txt,$i,$color) {
    ...
    echo '</label>
            ...
            <td class="st_td">
	               <div id="colorSelector" class="colorSelector" style="float: left;"><div style="background-color: ' . esc_attr($color) . '"></div></div>
	               <input type="hidden" value="' . esc_attr($color) . '" name="styles[' . intval($i) . ']" roll="' . intval($i) . '"  id="elem-' . intval($i) . '" />
            </td>
        </tr>
	';
}
    ...
    echo '</label>
             ...
             <td class="st_td">
            	<div class="size_container">
	            	<input class="size_input" type="text" value="' . esc_attr($size) . '" name="styles[' . intval($i) . ']" readonly="readonly" roll="' . intval($i) . '" id="elem-' . intval($i) . '" />
	            	<div class="size_arrows">
	            		<div class="size_up" maxval="' . esc_attr($max) . '" title="Up"></div>
	            		<div class="size_down" minval="' . esc_attr($min) . '" title="Down"></div>
	            	</div>
...
function echo_textarea_tr($txt,$i,$value,$desc) {
    echo '
		<tr>
            <td colspan="2">
	            	<textarea style="width: 330px;height: 350px;resize:none;" name="styles[' . intval($i) . ']" id="elem-' . intval($i) . '">' . esc_html($value) . '</textarea>
            		' . esc_html($desc) . '
            </td>
        </tr>';
}
...
function seperate_tr($txt,$title='',$roll='') {
    echo '<tr><td colspan="2"><div class="sep_td_wrapper"><div id="scroll_to_' . esc_attr($roll) . '" class="sep_td" title="' . esc_attr($title) . '">' . esc_html($txt) . '</div></div></td></tr>';
}
            ...
            <table style="width: 100%;height: 100%;text-decoration: none;">
                <tr>
                    <td align="center" valign="middle">
                        <img src="<?php echo esc_url(plugins_url( '../images/shopping_cart.png' , __FILE__ ));?>" /><br />
                        Buy Pro Version
                    </td>
                </tr>
            </table>
                ...
                <div class="contactformgenerator_wrapper_inner " >
                    ...
                    <div id="cfg_main_view_inner">
                        ...
                        <div class="contactformgenerator_header <?php if($styles[600] == 0) echo 'cfg_transparent'; ?>">
                            <div class="contactformgenerator_title <?php echo esc_attr($styles[510]);?>">
			 				<span style="position:relative;display: inline-block;">
			 					Contact Us
			 					<img roll="1" style="top: 2px;right: -23px;;" title="Edit: Top Text" src="<?php echo $plugin_path;?>/includes/assets/images/tmp_edit4.png" class="navigate_to_option"/>
			 				</span>
                            </div>
                            <div  class="contactformgenerator_pre_text <?php echo esc_attr($styles[511]);?>">
			 				<span style="position:relative;display: inline-block;">
			 					Contact us, if you have any questions
									<img roll="2" style="top: -2px;right: -23px;" title="Edit: Pre Text" src="<?php echo $plugin_path;?>/includes/assets/images/tmp_edit4.png" class="navigate_to_option" />
			 				</span>
                            </div>

                            <img roll="0" style="top: -7px;right: -7px;" title="Edit: Main Box" src="<?php echo $plugin_path;?>/includes/assets/images/tmp_edit4.png" class="navigate_to_option"/>
                        </div>
                        ...
                        <div class="contactformgenerator_body <?php if($styles[610] == 0) echo 'cfg_transparent'; ?>">
                            ...
                            <div class="contactformgenerator_field_box"><div class="contactformgenerator_field_box_inner">
                                    <label class="contactformgenerator_field_name <?php echo esc_attr($styles[512]);?>" for="name_0_1">
				 				<span class="cfg_label_txt_wrapper">
				 					Text Input <span class="contactformgenerator_field_required">*</span>
		 							<img roll="4" style="top: -2px;right: -50px;" title="Edit: Label Asterisk Symbol" src="<?php echo $plugin_path;?>/includes/assets/images/tmp_edit4.png" class="navigate_to_option" />
		 							<img roll="3" style="top: -2px;right: -34px;" title="Edit: Label Text" src="<?php echo $plugin_path;?>/includes/assets/images/tmp_edit4.png" class="navigate_to_option" />
				 				</span>
                                    </label>
                                    <div class="contactformgenerator_input_element contactformgenerator_required">
                                        <div class="cfg_input_dummy_wrapper">
				 					<span class="the-tooltip top right <?php echo $st = $styles[505] == '' ? 'white' : esc_attr($styles[505]);?>">
				 						<span class="tooltip_inner ">
				 							Tooltip text goes here!
		 									<img roll="12" style="top: 0px;right: -23px;" title="Edit: Tooltip Style" src="<?php echo $plugin_path;?>/includes/assets/images/tmp_edit4.png" class="navigate_to_option" />
				 						</span>
				 					</span>
                                            ...
                                            <img roll="5" style="top: 2px;right: -23px;" title="Edit: Text Inputs" src="<?php echo $plugin_path;?>/includes/assets/images/tmp_edit4.png" class="navigate_to_option" />
                                        </div>
                                    </div>
                                </div></div>
                            ...
                            <div class="contactformgenerator_field_box contactformgenerator_field_box_hovered"><div class="contactformgenerator_field_box_inner">
				 				...
				 				<span class="cfg_label_txt_wrapper">
				 					Hover State
				 					<img roll="13" style="top: -2px;right: -23px;" title="Edit: Label Text Focus State" src="<?php echo $plugin_path;?>/includes/assets/images/tmp_edit4.png" class="navigate_to_option" />
				 				</span>
                                    </label>
                                    <div class="contactformgenerator_input_element contactformgenerator_input_element_hovered">
                                        <div class="cfg_input_dummy_wrapper">
                                            ...
                                            <img roll="6" style="top: 2px;right: -23px;" title="Edit: Hover State" src="<?php echo $plugin_path;?>/includes/assets/images/tmp_edit4.png" class="navigate_to_option" />
                                        </div>
                                    </div>
                                </div></div>
                            ...
                            <div class="contactformgenerator_field_box contactformgenerator_error"><div class="contactformgenerator_field_box_inner">
				 				...
				 				<span class="cfg_label_txt_wrapper">
				 					Error state
				 					<img roll="14" style="top: -2px;right: -23px;" title="Edit: Label Text Error State" src="<?php echo $plugin_path;?>/includes/assets/images/tmp_edit4.png" class="navigate_to_option" />
				 				</span>
                                    </label>
                                    <div class="contactformgenerator_input_element contactformgenerator_required cfg_error_input">
                                        <div class="cfg_input_dummy_wrapper">
                                            ...
                                            <img roll="7" style="top: 2px;right: -23px;" title="Edit: Error State" src="<?php echo $plugin_path;?>/includes/assets/images/tmp_edit4.png" class="navigate_to_option" />
                                        </div>
                                    </div>
                                </div></div>
                            ...
                            <div class="contactformgenerator_field_box" style="margin: 0 !important;position: relative;"><div class="contactformgenerator_field_box_inner" style="width: 100%">
                                    <div class="contactformgenerator_heading <?php echo esc_attr($styles[530]);?>">
                                    ...
                                    <img roll="19" style="top: 5px;right: -15px;" title="Edit: Heading Styles" src="<?php echo $plugin_path;?>/includes/assets/images/tmp_edit4.png" class="navigate_to_option" />
                                </div></div>

                            <div class="contactformgenerator_field_box"><div class="contactformgenerator_field_box_textarea_inner">
                                    <label class="contactformgenerator_field_name <?php echo esc_attr($styles[512]);?>" for="text-area_0_5"><span class="cfg_label_txt_wrapper">Textarea</span></label>
                                    <div class="contactformgenerator_input_element cfg_textarea_wrapper contactformgenerator_required">
                                        <div class="cfg_textarea_dummy_wrapper">
                                            ...
                                            <img roll="11" style="top: 2px;right: -23px;" title="Edit: Textarea Styles" src="<?php echo $plugin_path;?>/includes/assets/images/tmp_edit4.png" class="navigate_to_option" />
                                        </div>
                                    </div>
                                </div></div>
                        ...
                        <div class="contactformgenerator_footer <?php if($styles[617] == 0) echo 'cfg_transparent'; ?>">
                            <div class="contactformgenerator_submit_wrapper">
                                <div class="cfg_button_holder" style="position: relative;display: inline-block;">
                                    <input type="button" value="Send" class="contactformgenerator_send <?php echo esc_attr($styles[515]);?>" roll="1" />
                                    <img roll="9" style="top: 2px;right: -18px;" title="Edit: Send Button" src="<?php echo $plugin_path;?>/includes/assets/images/tmp_edit4.png" class="navigate_to_option" />
                                ...
                                <div class="cfg_button_holder" style="position: relative;display: inline-block;margin: 0 30px;">
                                    ...
                                    <img roll="10" style="top: 2px;right: -18px;" title="Edit: Send Button Hover State" src="<?php echo $plugin_path;?>/includes/assets/images/tmp_edit4.png" class="navigate_to_option" />
                                    <div class="contactformgenerator_clear"></div>
                                </div>
                            </div>
                            <div class="contactformgenerator_clear"></div>
                        ...
                        <div class="contactformgenerator_body">
                            <div class="cfg_preview_left_col" >
                                ...
                                <div class="contactformgenerator_field_box"><div class="contactformgenerator_field_box_inner" style="width: 100%">
                                        <div class="cfg_sections_wrapper cfg_sections_template_<?php echo esc_attr($styles[552]);?>">
                                            <div class="cfg_content_element">
                                                ...
                                                <div class="cfg_content_element_icon_wrapper" style="width: 120px;position: relative">
                                                    ...
                                                    <img roll="20" style="top: 3px;right: 5px;" title="Edit: Sections Styles" src="<?php echo $plugin_path;?>/includes/assets/images/tmp_edit4.png" class="navigate_to_option" />
                                                </div>
                                            </div>
                            ...
                            <div class="cfg_preview_right_col">
                                ...
                                <div class="contactformgenerator_field_box"><div class="contactformgenerator_field_box_inner" style="width: 82%">
                                        ...
                                        <div class="contactformgenerator_input_element contactformgenerator_required">
                                            <div class="cfg_input_dummy_wrapper">
                                                ...
                                                <img class="ui-datepicker-trigger cfg_datepicker_icon" data_src="<?php echo $plugin_path;?>/includes/assets/images/datepicker/"  src="<?php echo $plugin_path;?>/includes/assets/images/datepicker/style-<?php echo esc_attr($styles[563]);?>.png" alt="Select date" title="Select date">
                                                <img roll="21" style="top: 2px;right: -50px;" title="Edit: Text Inputs" src="<?php echo $plugin_path;?>/includes/assets/images/tmp_edit4.png" class="navigate_to_option" />
                                            </div>
                                        </div>
                                    </div></div>

                                <div class="contactformgenerator_field_box "><div class="contactformgenerator_field_box_inner" style="width: 100%">
                                        <label class="contactformgenerator_field_name <?php echo esc_attr($styles[512]);?>" >
						 				<span class="cfg_label_txt_wrapper">
						 					File Upload
						 				</span>
                                        </label>
                                        <div style="position: relative">
                                            <input style="float: none;" type="button" value="Select Files..." class="cfg_fileupload contactformgenerator_send <?php echo esc_attr($styles[515]);?>" roll="1" />
                                            <img roll="25" style="top: 8px;right: -5px;" title="Edit: Send Button" src="<?php echo $plugin_path;?>/includes/assets/images/tmp_edit4.png" class="navigate_to_option" />
                                        </div>
                                    </div></div>
                                ...
                                <div class="contactformgenerator_field_box" style="margin-top: 20px !important; "><div class="contactformgenerator_field_box_inner" style="width: 100%">
						 			<span style="position: relative;margin-right: 25px;">
						 				...
						 				<img roll="22" style="top: -1px;right: -15px;" title="Edit: Links" src="<?php echo $plugin_path;?>/includes/assets/images/tmp_edit4.png" class="navigate_to_option" />
						 			</span>
                                        <span style="position: relative;">
						 				<a href="#" onclick="return false;" class="cfg_link_hovered">Hover state.</a>
						 				<img roll="23" style="top: -1px;right: -19px;" title="Edit: Links Hover State" src="<?php echo $plugin_path;?>/includes/assets/images/tmp_edit4.png" class="navigate_to_option" />
						 			</span>
                                    </div></div>
                                ...
                                <div class="contactformgenerator_field_box" style="margin-top: 20px !important; "><div class="contactformgenerator_field_box_inner" style="width: 100%">
						 			<span style="position: relative;">
						 				...
						 				<img roll="24" style="top: -1px;right: -19px;" title="Edit: Links" src="<?php echo $plugin_path;?>/includes/assets/images/tmp_edit4.png" class="navigate_to_option" />
						 			</span>
                                    </div></div>
...
<style>
    ...
    .contactformgenerator_wrapper {
        <?php
            $border_width = isset($styles[2]) ? intval($styles[2]) : 0;
            $border_style = isset($styles[3]) ? esc_attr($styles[3]) : 'solid';
            $border_color = isset($styles[1]) ? esc_attr($styles[1]) : '#000000';
            $bg_color = isset($styles[0]) ? esc_attr($styles[0]) : '#ffffff';

            echo "border: {$border_width}px {$border_style} {$border_color};\n";
            echo "background-color: {$bg_color};\n";

            if (isset($styles[627]) && $styles[627] == '1') {
                $start_color = $bg_color;
                $end_color = isset($styles[130]) ? esc_attr($styles[130]) : $bg_color;

                echo "filter: progid:DXImageTransform.Microsoft.gradient(startColorstr='{$start_color}', endColorstr='{$end_color}'); /* for IE */\n";
                echo "background: -webkit-gradient(linear, 0% 0%, 0% 100%, from({$start_color}), to({$end_color}));/* Safari 4-5, Chrome 1-9 */\n";
                echo "background: -webkit-linear-gradient(top, {$start_color}, {$end_color}); /* Safari 5.1, Chrome 10+ */\n";
                echo "background: -moz-linear-gradient(top, {$start_color}, {$end_color});/* Firefox 3.6+ */\n";
                echo "background: -ms-linear-gradient(top, {$start_color}, {$end_color});/* IE 10 */\n";
                echo "background: -o-linear-gradient(top, {$start_color}, {$end_color});/* Opera 11.10+ */\n";
            }
        ?>

        <?php
            // Sanitize individual values
            $shadow_inset = esc_attr($styles[9]);  // Typically 'inset' or empty
            $shadow_x = intval($styles[10]);
            $shadow_y = intval($styles[11]);
            $shadow_blur = intval($styles[12]);
            $shadow_spread = intval($styles[13]);
            $shadow_color = esc_attr($styles[8]);  // Should be a valid CSS color

            $radius_top_left = intval($styles[4]);
            $radius_top_right = intval($styles[5]);
            $radius_bottom_left = intval($styles[6]);
            $radius_bottom_right = intval($styles[7]);
        ?>

        -moz-box-shadow: <?php echo "{$shadow_inset} {$shadow_x}px {$shadow_y}px {$shadow_blur}px {$shadow_spread}px {$shadow_color}"; ?>;
        -webkit-box-shadow: <?php echo "{$shadow_inset} {$shadow_x}px {$shadow_y}px {$shadow_blur}px {$shadow_spread}px {$shadow_color}"; ?>;
        box-shadow: <?php echo "{$shadow_inset} {$shadow_x}px {$shadow_y}px {$shadow_blur}px {$shadow_spread}px {$shadow_color}"; ?>;

        -webkit-border-top-left-radius: <?php echo $radius_top_left; ?>px;
        -moz-border-radius-topleft: <?php echo $radius_top_left; ?>px;
        border-top-left-radius: <?php echo $radius_top_left; ?>px;

        -webkit-border-top-right-radius: <?php echo $radius_top_right; ?>px;
        -moz-border-radius-topright: <?php echo $radius_top_right; ?>px;
        border-top-right-radius: <?php echo $radius_top_right; ?>px;

        -webkit-border-bottom-left-radius: <?php echo $radius_bottom_left; ?>px;
        -moz-border-radius-bottomleft: <?php echo $radius_bottom_left; ?>px;
        border-bottom-left-radius: <?php echo $radius_bottom_left; ?>px;

        -webkit-border-bottom-right-radius: <?php echo $radius_bottom_right; ?>px;
        -moz-border-radius-bottomright: <?php echo $radius_bottom_right; ?>px;
        border-bottom-right-radius: <?php echo $radius_bottom_right; ?>px;
    <?php
	...
	?>
        ...
        color: <?php echo esc_attr($styles[587]);?>;
        font-size: <?php echo intval($styles[588]);?>px;
    }

    <?php
        $box_color = $styles[15];
        $box_offset_x = intval($styles[16]);
        $box_offset_y = intval($styles[17]);
        $box_blur = intval($styles[18]);
        $box_spread = intval($styles[19]);
        $box_inset = $styles[14];
    ?>
    .contactformgenerator_wrapper:hover {
        -moz-box-shadow: <?php echo "$box_color {$box_offset_x}px {$box_offset_y}px {$box_blur}px {$box_spread}px $box_inset"; ?>;
        -webkit-box-shadow: <?php echo "$box_color {$box_offset_x}px {$box_offset_y}px {$box_blur}px {$box_spread}px $box_inset"; ?>;
        box-shadow: <?php echo "$box_color {$box_offset_x}px {$box_offset_y}px {$box_blur}px {$box_spread}px $box_inset"; ?>;
    }
    ...
    <?php
       // Sanitize padding and border
       $pad_top    = intval($styles[603]);
       $pad_right  = intval($styles[604]);
       $pad_bottom = intval($styles[605]);
       $pad_left   = intval($styles[606]);

       $border_width = intval($styles[607]);
       $border_style = esc_attr($styles[608]); // e.g. solid, dashed
       $border_color = esc_attr($styles[609]); // e.g. #ccc or rgb(...)

       $use_gradient = ($styles[600] == '1');
       $color_start  = esc_attr($styles[601]);
       $color_end    = esc_attr($styles[602]);
       ?>

    .contactformgenerator_header {
        padding: <?php echo "$pad_top"."px $pad_right"."px $pad_bottom"."px $pad_left"."px"; ?>;
        border-bottom: <?php echo "$border_width"."px $border_style $border_color"; ?>;

    <?php if ($use_gradient): ?>
        background-color: <?php echo $color_start; ?>;
        filter: progid:DXImageTransform.Microsoft.gradient(startColorstr='<?php echo $color_start; ?>', endColorstr='<?php echo $color_end; ?>'); /* for IE */
        background: -webkit-gradient(linear, 0% 0%, 0% 100%, from(<?php echo $color_start; ?>), to(<?php echo $color_end; ?>));
        background: -webkit-linear-gradient(top, <?php echo $color_start; ?>, <?php echo $color_end; ?>);
        background: -moz-linear-gradient(top, <?php echo $color_start; ?>, <?php echo $color_end; ?>);
        background: -ms-linear-gradient(top, <?php echo $color_start; ?>, <?php echo $color_end; ?>);
        background: -o-linear-gradient(top, <?php echo $color_start; ?>, <?php echo $color_end; ?>);
    <?php endif; ?>

    }

    .contactformgenerator_body {
        padding: <?php echo intval($styles[613]);?>px  <?php echo intval($styles[614]);?>px <?php echo intval($styles[615]);?>px <?php echo intval($styles[616]);?>px;

    <?php
       if ($styles[610] == '1') {
           $start_color = esc_attr($styles[611]);
           $end_color = esc_attr($styles[612]);
       ?>
            background-color: <?php echo $start_color; ?>;
            filter: progid:DXImageTransform.Microsoft.gradient(startColorstr='<?php echo $start_color; ?>', endColorstr='<?php echo $end_color; ?>'); /* for IE */
            background: -webkit-gradient(linear, 0% 0%, 0% 100%, from(<?php echo $start_color; ?>), to(<?php echo $end_color; ?>)); /* Safari 4-5, Chrome 1-9 */
            background: -webkit-linear-gradient(top, <?php echo $start_color; ?>, <?php echo $end_color; ?>); /* Safari 5.1, Chrome 10+ */
            background: -moz-linear-gradient(top, <?php echo $start_color; ?>, <?php echo $end_color; ?>); /* Firefox 3.6+ */
            background: -ms-linear-gradient(top, <?php echo $start_color; ?>, <?php echo $end_color; ?>); /* IE 10 */
            background: -o-linear-gradient(top, <?php echo $start_color; ?>, <?php echo $end_color; ?>); /* Opera 11.10+ */
        <?php } ?>
    }

    .contactformgenerator_footer {
        padding: <?php echo intval($styles[620]);?>px  <?php echo intval($styles[621]);?>px <?php echo intval($styles[622]);?>px <?php echo intval($styles[623]);?>px;
        border-top: <?php echo intval($styles[624]);?>px<?php echo intval($styles[625]);?> <?php echo intval($styles[626]);?>;


        <?php
             if ($styles[617] == '1') {
                 $start_color = esc_attr($styles[618]);
                 $end_color = esc_attr($styles[619]);
         ?>
            background-color: <?php echo $start_color; ?>;
            filter: progid:DXImageTransform.Microsoft.gradient(startColorstr='<?php echo $start_color; ?>', endColorstr='<?php echo $end_color; ?>'); /* for IE */
            background: -webkit-gradient(linear, 0% 0%, 0% 100%, from(<?php echo $start_color; ?>), to(<?php echo $end_color; ?>)); /* Safari 4-5, Chrome 1-9 */
    ...
    .contactformgenerator_title {
        ...
        text-shadow: <?php echo intval($styles[28]); ?>px
                <?php echo intval($styles[29]); ?>px
                <?php echo intval($styles[30]); ?>px<?php echo esc_attr($styles[27]); ?>;
    <?php
    ...
    .contactformgenerator_field_name {
        color: <?php echo esc_attr($styles[31]); ?>;
        font-size: <?php echo intval($styles[32]); ?>px;
        font-style: <?php echo esc_attr($styles[34]); ?>;
        font-weight: <?php echo esc_attr($styles[33]); ?>;
        text-align: <?php echo esc_attr($styles[36]); ?>;
        text-decoration: <?php echo esc_attr($styles[35]); ?>;
        text-shadow: <?php echo intval($styles[38]); ?>px
                    <?php echo intval($styles[39]); ?>px
                    <?php echo intval($styles[40]); ?>px<?php echo esc_attr($styles[37]); ?>;
        margin: <?php echo intval($styles[215]); ?>px
                    <?php echo intval($styles[216]); ?>px
                    <?php echo intval($styles[217]); ?>px
                    <?php echo intval($styles[218]); ?>px;
    <?php
    ...
    .contactformgenerator_field_required {
        ...
        text-shadow: <?php echo intval($styles[47]); ?>px
                    <?php echo intval($styles[48]); ?>px
                    <?php echo intval($styles[49]); ?>px<?php echo esc_attr($styles[46]); ?>;
    }
    .cfg_button_holder {
        float: <?php echo esc_attr($styles[212]);?>;
    }

    .contactformgenerator_send {
    <?php
    $bg_start = esc_attr($styles[91]);
    $bg_end = esc_attr($styles[50]);
    $boxColor = esc_attr($styles[94]);
    $box_x = intval($styles[95]);
    $box_y = intval($styles[96]);
    $box_blur = intval($styles[97]);
    $box_spread = intval($styles[98]);
    $box_inset = intval($styles[99]);


    $border_top_left = intval($styles[102]);
    $border_top_right = intval($styles[103]);
    $border_bottom_left = intval($styles[104]);
    $border_bottom_right = intval($styles[105]);
?> background-color: <?php echo $bg_start;?>;
        filter: progid:DXImageTransform.Microsoft.gradient(startColorstr='<?php echo $bg_start;?>', endColorstr='<?php echo $bg_start;?>'); /* for IE */
        background: -webkit-gradient(linear, 0% 0%, 0% 100%, from(<?php echo $bg_start;?>), to(<?php echo $bg_start;?>)); /* Safari 4-5, Chrome 1-9 */
        background: -webkit-linear-gradient(top, <?php echo $bg_start;?>, <?php echo $bg_start;?>); /* Safari 5.1, Chrome 10+ */
        background: -moz-linear-gradient(top, <?php echo $bg_start;?>, <?php echo $bg_start;?>); /* Firefox 3.6+ */
        background: -ms-linear-gradient(top, <?php echo $bg_start;?>, <?php echo $bg_start;?>); /* IE 10 */
        background: -o-linear-gradient(top, <?php echo $bg_start;?>, <?php echo $bg_start;?>); /* Opera 11.10+ */

        padding: <?php echo $styles[92];?>px <?php echo $styles[93];?>px;
        -moz-box-shadow: <?php echo  $box_x;?> <?php echo  $box_y;?>px <?php echo $box_blur;?>px <?php echo $box_spread;?>px <?php echo $box_inset;?>px<?php echo $boxColor;?>;
        -webkit-box-shadow: <?php echo  $box_x;?> <?php echo  $box_y;?>px <?php echo $box_blur;?>px <?php echo $box_spread;?>px <?php echo $box_inset;?>px<?php echo $boxColor;?>;
        box-shadow: <?php echo  $box_x;?> <?php echo  $box_y;?>px <?php echo $box_blur;?>px <?php echo $box_spread;?>px <?php echo $box_inset;?>px<?php echo $boxColor;?>;
        border-style: <?php echo esc_attr($styles[127]);?>;
        border-width: <?php echo intval($styles[101]);?>px;
        border-color: <?php echo esc_attr($styles[100]);?>;

        -webkit-border-top-left-radius: <?php echo $border_top_left;?>px;
        -moz-border-radius-topleft: <?php echo $border_top_left;?>px;
        border-top-left-radius: <?php echo $border_top_left;?>px;

        -webkit-border-top-right-radius: <?php echo $border_top_right;?>px;
        -moz-border-radius-topright: <?php echo $border_top_right;?>px;
        border-top-right-radius: <?php echo $border_top_right;?>px;

        -webkit-border-bottom-left-radius: <?php echo $border_bottom_left;?>px;
        -moz-border-radius-bottomleft: <?php echo $border_bottom_left;?>px;
        border-bottom-left-radius: <?php echo $border_bottom_left;?>px;

        -webkit-border-bottom-right-radius: <?php echo $border_bottom_right;?>px;
        -moz-border-radius-bottomright: <?php echo $border_bottom_right;?>px;
        border-bottom-right-radius: <?php echo $border_bottom_right;?>px;
        float: <?php echo esc_attr($styles[212]);?>;

        font-size: <?php echo intval($styles[107]);?>px;
        color: <?php echo $styles[106];?>;
        font-style: <?php echo esc_attr($styles[109]);?>;
        font-weight: <?php echo esc_attr($styles[108]);?>;
        text-decoration: <?php echo esc_attr($styles[110]);?>;
        text-shadow: <?php echo intval($styles[114]);?>px <?php echo intval($styles[115]);?>px <?php echo intval($styles[116]);?>px<?php echo intval($styles[113]);?>;
    <?php
    ...
    <?php
    $background_start = esc_attr($styles[51]);
    $background_end = esc_attr($styles[52]);
    $box_inset = esc_attr($styles[118]);
    $box_offset_x = intval($styles[119]);
    $box_offset_y = intval($styles[120]);
    $box_blur = intval($styles[121]);
    $box_spread = intval($styles[122]);
    $box_color = esc_attr($styles[117]);
?> background-color: <?php echo $background_start;?>;
        filter: progid:DXImageTransform.Microsoft.gradient(startColorstr='<?php echo $background_start;?>', endColorstr='<?php echo $background_end;?>'); /* for IE */
        background: -webkit-gradient(linear, 0% 0%, 0% 100%, from(<?php echo $background_start;?>), to(<?php echo $background_end;?>)); /* Safari 4-5, Chrome 1-9 */
        background: -webkit-linear-gradient(top, <?php echo $background_start;?>, <?php echo $background_end;?>); /* Safari 5.1, Chrome 10+ */
        background: -moz-linear-gradient(top, <?php echo $background_start;?>, <?php echo $background_end;?>); /* Firefox 3.6+ */
        background: -ms-linear-gradient(top, <?php echo $background_start;?>, <?php echo $background_end;?>); /* IE 10 */
        background: -o-linear-gradient(top, <?php echo $background_start;?>, <?php echo $background_end;?>); /* Opera 11.10+ */

        color: <?php echo esc_attr($styles[124]);?>;
        text-shadow: <?php echo intval($styles[114]);?>px <?php echo intval($styles[115]);?>px <?php echo intval($styles[116]);?>px<?php echo esc_attr($styles[125]);?>;
        -moz-box-shadow: <?php echo $box_inset;?> <?php echo $box_offset_x;?>px <?php echo $box_offset_y ;?>px <?php echo $box_blur;?>px <?php echo $box_spread;?>px<?php echo $box_color ;?>;
        -webkit-box-shadow: <?php echo $box_inset;?> <?php echo $box_offset_x;?>px <?php echo $box_offset_y ;?>px <?php echo $box_blur;?>px <?php echo $box_spread;?>px<?php echo $box_color ;?>;
        box-shadow: <?php echo $box_inset;?> <?php echo $box_offset_x;?>px <?php echo $box_offset_y ;?>px <?php echo $box_blur;?>px <?php echo $box_spread;?>px<?php echo $box_color ;?>;
        border-style: <?php echo esc_attr($styles[127]);?>;
        border-width: <?php echo intval($styles[101]);?>px;
        border-color: <?php echo esc_attr($styles[126]);?>;
    }
    .cfg_fileupload {
    ...
    .contactformgenerator_input_element {
        ...
        <?php
            $box_start = $styles[132];
            $box_end = $styles[133];
            $box_border_color = $styles[134];
            $box_border_width = intval($styles[135]);
            $box_border_style = $styles[136];

            $radius_tl = intval($styles[137]);
            $radius_tr = intval($styles[138]);
            $radius_bl = intval($styles[139]);
            $radius_br = intval($styles[140]);

            $box_shadow_color = $styles[141];
            $box_shadow_type = $styles[142];
            $box_shadow_x = intval($styles[143]);
            $box_shadow_y = intval($styles[144]);
            $box_shadow_blur = intval($styles[145]);
            $box_shadow_spread= intval($styles[146]);
        ?>

        background-color: <?php echo $box_start; ?>;
        filter: progid:DXImageTransform.Microsoft.gradient(startColorstr='<?php echo $box_start; ?>', endColorstr='<?php echo $box_end; ?>'); /* for IE */
        background: -webkit-gradient(linear, 0% 0%, 0% 100%, from(<?php echo $box_start; ?>), to(<?php echo $box_end; ?>)); /* Safari 4-5, Chrome 1-9 */
        background: -webkit-linear-gradient(top, <?php echo $box_start; ?>, <?php echo $box_end; ?>); /* Safari 5.1, Chrome 10+ */
        background: -moz-linear-gradient(top, <?php echo $box_start; ?>, <?php echo $box_end; ?>); /* Firefox 3.6+ */
        background: -ms-linear-gradient(top, <?php echo $box_start; ?>, <?php echo $box_end; ?>); /* IE 10 */
        background: -o-linear-gradient(top, <?php echo $box_start; ?>, <?php echo $box_end; ?>); /* Opera 11.10+ */

        -moz-box-shadow: <?php echo "$box_shadow_type {$box_shadow_x}px {$box_shadow_y}px {$box_shadow_blur}px {$box_shadow_spread}px $box_shadow_color"; ?>;
        -webkit-box-shadow: <?php echo "$box_shadow_type {$box_shadow_x}px {$box_shadow_y}px {$box_shadow_blur}px {$box_shadow_spread}px $box_shadow_color"; ?>;
        box-shadow: <?php echo "$box_shadow_type {$box_shadow_x}px {$box_shadow_y}px {$box_shadow_blur}px {$box_shadow_spread}px $box_shadow_color"; ?>;

        border-style: <?php echo $box_border_style; ?>;
        border-width: <?php echo $box_border_width; ?>px;
        border-color: <?php echo $box_border_color; ?>;

        -webkit-border-top-left-radius: <?php echo $radius_tl; ?>px;
        -moz-border-radius-topleft: <?php echo $radius_tl; ?>px;
        border-top-left-radius: <?php echo $radius_tl; ?>px;

        -webkit-border-top-right-radius: <?php echo $radius_tr; ?>px;
        -moz-border-radius-topright: <?php echo $radius_tr; ?>px;
        border-top-right-radius: <?php echo $radius_tr; ?>px;

        -webkit-border-bottom-left-radius: <?php echo $radius_bl; ?>px;
        -moz-border-radius-bottomleft: <?php echo $radius_bl; ?>px;
        border-bottom-left-radius: <?php echo $radius_bl; ?>px;

        -webkit-border-bottom-right-radius: <?php echo $radius_br; ?>px;
        -moz-border-radius-bottomright: <?php echo $radius_br; ?>px;
        border-bottom-right-radius: <?php echo $radius_br; ?>px;
    ...
    .contactformgenerator_input_element_hovered {
        <?php
            // Background colors
            $back_start = esc_attr($styles[157]);
            $back_end   = esc_attr($styles[158]);

            // Box shadow
            $shadow_inset = esc_attr($styles[162]);
            $shadow_color = esc_attr($styles[163]);
            $shadow_x = intval($styles[164]);
            $shadow_y = intval($styles[165]);
            $shadow_blur = intval($styles[166]);
            $shadow_spread = intval($styles[167]);
        ?>
        background-color: <?php echo $back_start;?>;
        filter: progid:DXImageTransform.Microsoft.gradient(startColorstr='<?php echo $back_start;?>', endColorstr='<?php echo $back_end;?>'); /* for IE */
        background: -webkit-gradient(linear, 0% 0%, 0% 100%, from(<?php echo $back_start;?>), to(<?php echo $back_end;?>)); /* Safari 4-5, Chrome 1-9 */
        background: -webkit-linear-gradient(top, <?php echo $back_start;?>, <?php echo $back_end;?>); /* Safari 5.1, Chrome 10+ */
        background: -moz-linear-gradient(top, <?php echo $back_start;?>, <?php echo $back_end;?>); /* Firefox 3.6+ */
        background: -ms-linear-gradient(top, <?php echo $back_start;?>, <?php echo $back_end;?>); /* IE 10 */
        background: -o-linear-gradient(top, <?php echo $back_start;?>, <?php echo $back_end;?>); /* Opera 11.10+ */

        -moz-box-shadow: <?php echo $shadow_color ;?> <?php echo $shadow_x;?>px <?php echo $shadow_y;?>px <?php echo $shadow_blur;?>px <?php echo $shadow_spread;?>px<?php echo $shadow_inset;?>;
        -webkit-box-shadow: <?php echo $shadow_color ;?> <?php echo $shadow_x;?>px <?php echo $shadow_y;?>px <?php echo $shadow_blur;?>px <?php echo $shadow_spread;?>px<?php echo $shadow_inset;?>;
        box-shadow: <?php echo $shadow_color;?> <?php echo $shadow_x;?>px <?php echo $shadow_y;?>px <?php echo $shadow_blur;?>px <?php echo $shadow_spread;?>px<?php echo $shadow_inset;?>;
        border-style: <?php echo esc_attr($styles[136]);?>;
        border-width: <?php echo intval($styles[135]);?>px;
        border-color: <?php echo esc_attr($styles[161]);?>;
    }
    ...
    .contactformgenerator_error .contactformgenerator_input_element,.contactformgenerator_error .contactformgenerator_input_element:hover {
        <?php
            ...
            $shadow_inset = esc_attr($styles[184]); // e.g., "inset" or empty
            $shadow_color = esc_attr($styles[185]);
            $shadow_x = intval($styles[186]);
            $shadow_y = intval($styles[187]);
            $shadow_blur = intval($styles[188]);
            $shadow_spread = intval($styles[189]);

            $box_shadow = "{$shadow_color} {$shadow_x}px {$shadow_y}px {$shadow_blur}px {$shadow_spread}px{$shadow_inset}";
        ?>
        background-color: <?php echo $bg_start; ?>;
        filter: progid:DXImageTransform.Microsoft.gradient(startColorstr='<?php echo $bg_start; ?>', endColorstr='<?php echo $bg_end; ?>'); /* IE */
        background: -webkit-gradient(linear, 0% 0%, 0% 100%, from(<?php echo $bg_start; ?>), to(<?php echo $bg_end; ?>)); /* Safari 4-5 */
        background: -webkit-linear-gradient(top, <?php echo $bg_start; ?>, <?php echo $bg_end; ?>); /* Safari 5.1+ */
        background: -moz-linear-gradient(top, <?php echo $bg_start; ?>, <?php echo $bg_end; ?>); /* Firefox */
        background: -ms-linear-gradient(top, <?php echo $bg_start; ?>, <?php echo $bg_end; ?>); /* IE 10+ */
        background: -o-linear-gradient(top, <?php echo $bg_start; ?>, <?php echo $bg_end; ?>); /* Opera */
    ...
    .contactformgenerator_error input,.contactformgenerator_error input:hover, .contactformgenerator_error .focused input:hover, .contactformgenerator_error .focused input, .contactformgenerator_error textarea,.contactformgenerator_error textarea:hover {
        ...
        text-shadow: <?php echo intval($styles[181]);?>px <?php echo intval($styles[182]);?>px <?php echo intval($styles[183]);?>px<?php echo intval($styles[180]);?>;
    }

    .contactformgenerator_pre_text {
        margin-top: <?php echo intval($styles[190]);?>px;
        margin-bottom: <?php echo intval($styles[191]);?>px;
...
if($styles[501] == 'right') {?>
    ...
    .contactformgenerator_heading_inner {
        margin: <?php echo intval($styles[535]);?>px <?php echo intval($styles[536]);?>px <?php echo intval($styles[537]);?>px <?php echo intval($styles[538]);?>px;
    ...
    .contactformgenerator_heading {
        ...
        overflow: hidden;
         <?php
            $font_size = intval($styles[525]);
            $font_color = esc_attr($styles[524]);
            $font_style = esc_attr($styles[527]);
            $font_weight = esc_attr($styles[526]);
            $text_decoration = esc_attr($styles[528]);
            $text_shadow_color = esc_attr($styles[531]);
            $text_shadow_x = intval($styles[532]);
            $text_shadow_y = intval($styles[533]);
            $text_shadow_blur = intval($styles[534]);
            ...
            $bg_start = esc_attr($styles[541]);
            $bg_end = esc_attr($styles[542]);
        ?>
        ...
        text-shadow: <?php echo "$text_shadow_x"."px $text_shadow_y"."px $text_shadow_blur"."px $text_shadow_color"; ?>;

        margin: <?php echo $margin_top; ?>px 0 <?php echo $margin_bottom; ?>px 0;

        border-top: <?php echo "$border_top_width"."px $border_style $border_top_color"; ?>;
        border-right: <?php echo "$border_right_width"."px $border_style $border_right_color"; ?>;
        border-bottom: <?php echo "$border_bottom_width"."px $border_style $border_bottom_color"; ?>;
        border-left: <?php echo "$border_left_width"."px $border_style $border_left_color"; ?>;
    ...
    .cfg_content_element_label {
        font-size: <?php echo intval($styles[554]); ?>px;
        color: <?php echo esc_attr($styles[553]); ?>;
        font-style: <?php echo esc_attr($styles[556]); ?>;
        font-weight: <?php echo esc_attr($styles[555]); ?>;
        text-shadow: <?php
        echo intval($styles[559]) . 'px ' .
             intval($styles[560]) . 'px ' .
             intval($styles[561]) . 'px ' .
             esc_attr($styles[558]);
         ?>;
        border-bottom: <?php
             echo intval($styles[590]) . 'px ' .
             esc_attr($styles[591]) . ' ' .
             esc_attr($styles[592]);
        ?>;
        text-decoration: <?php echo esc_attr($styles[596]); ?>;
    }
    a.cfg_link {
        color: <?php echo esc_attr($styles[564]); ?>;
        font-style: <?php echo esc_attr($styles[566]); ?>;
        font-weight: <?php echo esc_attr($styles[565]); ?>;
        text-shadow: <?php
            echo intval($styles[571]) . 'px ' .
                 intval($styles[572]) . 'px ' .
                 intval($styles[573]) . 'px ' .
                 esc_attr($styles[570]);
            ?>;
        border-bottom: <?php
            echo intval($styles[567]) . 'px ' .
                 esc_attr($styles[568]) . ' ' .
                 esc_attr($styles[569]);
            ?>;
        text-decoration: <?php echo esc_attr($styles[594]); ?>;
    }
    ...
    a.cfg_link_hovered {
        color: <?php echo esc_attr($styles[574]); ?>;
        text-shadow: <?php
            echo intval($styles[577]) . 'px ' .
                 intval($styles[578]) . 'px ' .
                 intval($styles[579]) . 'px ' .
                 esc_attr($styles[576]);
            ?>;
        border-bottom: <?php
            echo intval($styles[567]) . 'px ' .
        ...
        font-style: <?php echo esc_attr($styles[566]); ?>;
        font-weight: <?php echo esc_attr($styles[565]); ?>;
        text-decoration: <?php echo esc_attr($styles[595]); ?>;
    ...
    .cfg_content_styling {
        color: <?php echo esc_attr($styles[580]); ?>;
        font-style: <?php echo esc_attr($styles[582]); ?>;
        font-weight: <?php echo esc_attr($styles[581]); ?>;
        text-shadow: <?php
            echo intval($styles[584]) . 'px ' .
                 intval($styles[585]) . 'px ' .
                 intval($styles[586]) . 'px ' .
                 esc_attr($styles[583]);
        ?>;
        text-decoration: <?php echo esc_attr($styles[593]); ?>;

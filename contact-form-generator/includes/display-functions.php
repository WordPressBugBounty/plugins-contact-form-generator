...
function cfgen_shortcode_function( $atts ) {
	...
	cfgen_enqueue_front_scripts($id);

	return cfgen_render_html($id);
}

add_shortcode( 'contactformgenerator', 'cfgen_shortcode_function' );

//add_action('template_redirect','cfgen_my_shortcode_head');
function cfgen_my_shortcode_head(){
	...
	if (is_array($matches) && $matches[2] == 'contactformgenerator') {
		...
		cfgen_enqueue_front_scripts($form_id);
...
function cfgen_enqueue_front_scripts($form_id) {

	global $wpdb;
	global $wpcfg_plugin_version;
	$version = $wpcfg_plugin_version;
	$db_version = $wpcfg_plugin_version;
	
	//get field types array
	$types_array = cfgen_get_types_array($form_id);

	$cssFile = plugin_dir_url( __FILE__ ).'assets/css/main.css';
	wp_enqueue_style('wpcfg-styles1', $cssFile, false, $version);

	$cssFile = plugin_dir_url( __FILE__ ).'assets/css/cfg-css-ui.css';
	wp_enqueue_style('wpcfg-styles2', $cssFile, false, $version);

	$cssFile = plugin_dir_url( __FILE__ ).'assets/css/cfg-scroll.css';
	wp_enqueue_style('wpcfg-styles3', $cssFile, false, $version);

	$cssFile = plugin_dir_url( __FILE__ ).'assets/css/template.css';
	wp_enqueue_style('wpcfg-styles4', $cssFile, false, $version);

	$cssFile = plugin_dir_url( __FILE__ ).'assets/css/cfg-tooltip.css';
	wp_enqueue_style('wpcfg-styles5', $cssFile, false, $version);

	$jsFile = plugin_dir_url( __FILE__ ).'assets/js/cfg-mousewheel.js';
	wp_enqueue_script('wpcfg-script1', $jsFile, array('jquery','jquery-ui-core','jquery-ui-widget','jquery-ui-draggable'), $version);

	$jsFile = plugin_dir_url( __FILE__ ).'assets/js/cfg-scroll.js';
	wp_enqueue_script('wpcfg-script2', $jsFile, array('jquery','jquery-ui-core','jquery-ui-widget','jquery-ui-draggable'), $version);

	wp_enqueue_script('wpcfg-script7' . $form_id, admin_url() . 'admin.php?page=cfg_forms&act=cfg_submit_data&holder=generate_js&id_form='.$form_id, array('jquery'), $version);

	$jsFile = plugin_dir_url( __FILE__ ).'assets/js/contactformgenerator.js';
	wp_enqueue_script('wpcfg-script-777', $jsFile, array('jquery','jquery-ui-core','jquery-effects-core','jquery-ui-datepicker'), $version);

	// 2.7.0
	$ajax_nonce = wp_create_nonce("wpgs_ajax_nonce_value");
	wp_localize_script(
		'wpcfg-script-777',
		'wpgfg_ajax_obj',
		array(
...
function cfgen_get_types_array($form_id) {
	global $wpdb;
...
function cfgen_get_form_data($form_id) {
	global $wpdb;
    ...
    $query = $wpdb->prepare(
        "
    SELECT
        sp.`id_template`, sp.name, sp.published, sp.top_text, sp.pre_text, sp.thank_you_text,
        sp.send_text, sp.send_new_text, sp.close_alert_text, sp.form_width, sp.redirect,
        sp.redirect_itemid, sp.redirect_url, sp.redirect_delay, sp.shake_count,
        sp.shake_distanse, sp.send_copy_enable, sp.send_copy_text, sp.shake_duration,
        sp.custom_css, st.styles 
    FROM
    ...
    ",
        1,         // published = 1
        $form_id   // ensure this is an integer
    );
...
// function wpcfg_make_statistics($d_) {

// 	if(!file_exists(ABSPATH.PLUGINDIR.'/contact-form-generator/wpcfg_v3.log') and is_writable(ABSPATH.PLUGINDIR.'/contact-form-generator')) {
...
// 		$fh = fopen(ABSPATH.PLUGINDIR.'/contact-form-generator/wpcfg_v3.log', 'a');
// 		fclose($fh);
// 	}
// }

function cfgen_get_field_data($form_id) {
	global $wpdb;
    ...
    $query = $wpdb->prepare(
        "
    SELECT
        sp.id, sp.name, sp.tooltip_text, sp.required, sp.ordering_field,
        sp.select_default_text, sp.show_parent_label, sp.select_no_match_text,
        sp.width, sp.field_margin_top, sp.select_show_scroll_after,
        sp.select_show_search_after, sp.upload_button_text, sp.upload_minfilesize,
        sp.upload_maxfilesize, sp.upload_acceptfiletypes, sp.upload_minfilesize_message,
        sp.upload_maxfilesize_message, sp.upload_acceptfiletypes_message,
        sp.captcha_wrong_message, sp.datepicker_date_format, sp.datepicker_animation,
        sp.datepicker_style, sp.datepicker_icon_style, sp.datepicker_show_icon,
        sp.datepicker_input_readonly, sp.datepicker_number_months,
        sp.datepicker_mindate, sp.datepicker_maxdate, sp.datepicker_changemonths,
        sp.datepicker_changeyears, sp.column_type, sp.custom_html, sp.google_maps,
        sp.heading, sp.recaptcha_site_key, sp.recaptcha_wrong_message,
        sp.recaptcha_theme, sp.recaptcha_type, sp.contact_data, sp.contact_data_width,
        sp.creative_popup, sp.creative_popup_embed,
        st.name as type
    FROM {$wpdb->prefix}cfg_fields sp
    JOIN {$wpdb->prefix}cfg_field_types st ON st.id = sp.id_type
    WHERE sp.published = %d
    AND sp.id_form = %d
    ORDER BY sp.ordering, sp.id
    ",
        1,           // published = 1
        $form_id     // form ID as integer
    );
...
function cfgen_get_rule_from_row($key,$row) {
...
function cfgen_get_last_rule_from_row($key,$row) {
	preg_match('/'.$key.'~([^\|]+)$/',$row,$m);
	if(isset($m[1]))
		return $m[1];
	else
...
function cfgen_process_sections($matched_sections) {
	global $wpcfg_section_width;
	global $wpcfg_section_id;
	$sections_width = (int)$wpcfg_section_width == 0 ? 110 : $wpcfg_section_width;
	$section_type = esc_attr($matched_sections[1]);
	$section_label = esc_attr($matched_sections[2]);
	$section_content = esc_html($matched_sections[3]);
...
function cfgen_process_txt($txt,$process_types_array) {
	global $wpcfg_heading_text_font_effect;
	if(in_array('sections',$process_types_array)) {
		$txt = preg_replace_callback('/{section icon=\"([a-zA-Z]*)\" label=\"(.*?)\"}(.*?){\/section}/s', 'cfgen_process_sections', $txt);
...
function cfgen_print_fields_array_html($field_data,$form_id) {
	global $wpdb;
	global $wpcfg_field_index;
	global $wpcfg_section_width;
	global $wpcfg_heading_text_font_effect;
	...
	$types_array = cfgen_get_types_array($form_id);
	$form_data = cfgen_get_form_data($form_id);
	...
	$tooltip_style = cfgen_get_rule_from_row(505,$styles_row);
	$tooltip_style = $tooltip_style == '' ? 'white' : $tooltip_style;

	$cfg_datepicker_style = cfgen_get_rule_from_row(562,$styles_row);
	$cfg_datepicker_icon_style = cfgen_get_rule_from_row(563,$styles_row);
	$cfg_global_icons_style = cfgen_get_rule_from_row(589,$styles_row);


	$top_text_font_effect = cfgen_get_rule_from_row(510,$styles_row);
	$wpcfg_heading_text_font_effect = cfgen_get_rule_from_row(530,$styles_row);
	$pre_text_font_effect = cfgen_get_rule_from_row(511,$styles_row);
	$label_text_font_effect = cfgen_get_rule_from_row(512,$styles_row);
	$label_hover_text_font_effect = cfgen_get_rule_from_row(513,$styles_row);
	$label_error_text_font_effect = cfgen_get_rule_from_row(514,$styles_row);
	$send_font_effect = cfgen_get_rule_from_row(515,$styles_row);
	$send_hover_font_effect = cfgen_get_rule_from_row(516,$styles_row);
	...
	foreach($field_data as $field) {
		...
		$field_name = cfgen_process_txt($field_name,array('num','popup'));
			...
			if (sizeof($childs_array) > 0)
			{
				...
				elseif($field_type == 'radio' || $field_type == 'checkbox') {
					...
					foreach ($childs_array as $key => $value)
					{
						...
						$current_label = cfgen_process_txt($current_label,array('num','popup'));
...
function cfgen_render_html($form_id) {
	...
	$str = 'domain=' . $domain . '&email=' . $m_  . '&name=' . $n_  . '&version=' . $wpcfg_plugin_version . '&plugin=cfg';
	$d_ = base64_encode($str);

	// wpcfg_make_statistics($d_);

	$browser_data = cfgen_getBrowser();
	$browser = $browser_data["name"] . ' ' . $browser_data["version"];
	$op_s = cfgen_getOS();

	$types_array = cfgen_get_types_array($form_id);
	$form_data = cfgen_get_form_data($form_id);
	$field_data = cfgen_get_field_data($form_id);
	...
	$scrollbar_popup_style = cfgen_get_rule_from_row(629,$styles_row);
	$scrollbar_popup_style = $scrollbar_popup_style == '' ? 'inset-2-dark' : $scrollbar_popup_style;
	$scrollbar_content_style = cfgen_get_rule_from_row(630,$styles_row);
	$scrollbar_content_style = $scrollbar_content_style == '' ? 'inset-2-dark' : $scrollbar_content_style;
	$custom_js = cfgen_get_last_rule_from_row(628,$styles_row);

	$cfg_global_icons_style = cfgen_get_rule_from_row(589,$styles_row);
	$cfg_sections_icons_style = cfgen_get_rule_from_row(552,$styles_row);

	$top_text_font_effect = cfgen_get_rule_from_row(510,$styles_row);
	$heading_text_font_effect = cfgen_get_rule_from_row(530,$styles_row);
	$pre_text_font_effect = cfgen_get_rule_from_row(511,$styles_row);
	$label_text_font_effect = cfgen_get_rule_from_row(512,$styles_row);
	$label_hover_text_font_effect = cfgen_get_rule_from_row(513,$styles_row);
	$label_error_text_font_effect = cfgen_get_rule_from_row(514,$styles_row);
	...
	$REMOTE_ADDR = cfgen_get_ip();

	$toptxt = $form_data["top_text"];
	$pretxt = stripcslashes($form_data["pre_text"]);
	$pretxt = cfgen_process_txt($pretxt,array('num','popup'));
	 						...
	 						foreach($fields_final_array as $k => $field_columns_array) {
	 							...
	 							if(isset($field_columns_array['0'])) {
		 							...
		 							echo '<div class="contactformgenerator_clear"></div><div class="cfg_field_box_wrapper cfg_field_box_wrapper_0 cfg_field_box_animation_state_1">';
		 								if(isset($field_columns_array['0'])) {
		 									cfgen_print_fields_array_html($field_columns_array['0'],$form_id);
		 								}
		 							echo '</div>';
		 						}
	 						}
